...
class TestAgentParser(unittest.TestCase):
    ...
    def test_parse_structured_cli_output(self):
        ...
        │ Tier: Pro Developer (Tier 2)                                              │
        ├───────────────────────┬──────────────┬──────────────┬─────────────────────┤
        │ Model Name            │ Requests/Min │ Daily Quota  │ Resets In           │
        ├───────────────────────┼──────────────┼──────────────┼─────────────────────┤
        │ Gemini 3.5 Flash      │ 120 / 1000   │ 12% used     │ 03h 42m (06:00 UTC) │
        │ Gemini 3.5 Pro        │ 15 / 50      │ 30% used     │ 03h 42m (06:00 UTC) │
        │ Claude Sonnet 4.6     │ 40 / 200     │ 20% used     │ 08h 15m (12:00 UTC) │
        ╰───────────────────────┴──────────────┴──────────────┴─────────────────────╯
        """
        ...
        flash = next(m for m in result["models"] if "flash" in m["model_id"])
